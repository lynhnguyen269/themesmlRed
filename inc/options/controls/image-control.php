...
if (class_exists('WP_Customize_Control')) {
	...
	class themesflat_ImagesControl extends WP_Customize_Control {
		public $type = 'image-control';
		...
		public function render_content() {
			$name = '_options-images-control-' . $this->id;
			...
			<div class="themesflat-options-control-field">
				<?php foreach ( $choices as $_value => $params ):  ?>
					...
					<label>
						<input type="file" value="<?php themesflat_esc_attr( $_value ) ?>" data-target="<?php themesflat_esc_attr( $name ) ?>" name="<?php themesflat_esc_attr( $name ) ?>" <?php $this->link();  checked( $this->value(), $_value ) ?> />
