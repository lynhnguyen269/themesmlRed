...
class Breadcrumb_Trail {
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => esc_html__( 'Browse:', 'redbiz' ),
			'home'                => esc_html__( 'Home', 'redbiz' ),
			'error_404'           => esc_html__( '404 Not Found', 'redbiz' ),
			'archives'            => esc_html__( 'Archives', 'redbiz' ),
			/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;', 'redbiz' ),
			/* Translators: %s is the page number. */
			'paged'               => esc_html__( 'Page %s', 'redbiz' ),
			/* Translators: Minute archive title. %s is the minute time format. */
			'archive_minute'      =>esc_html__( 'Minute %s', 'redbiz' ),
			/* Translators: Weekly archive title. %s is the week date format. */
			'archive_week'        => esc_html__( 'Week %s', 'redbiz' ),
	...
	public function themesflat_do_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'redbiz' ) ) );
	}
	...
	public function themesflat_do_minute_archive_items() {
		...
		/* Add the minute item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( _x( 'i', 'minute archives time format', 'redbiz' ) ) );
	}
	...
	public function themesflat_do_hour_archive_items() {
		...
		/* Add the hour item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( _x( 'g a', 'hour archives time format', 'redbiz' ) ) );
	}
	...
	public function themesflat_do_day_archive_items() {
		...
		$year  = sprintf( $this->args['labels']['archive_year'],  get_the_time( _x( 'Y', 'yearly archives date format',  'redbiz' ) ) );
		$month = sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'redbiz' ) ) );
		$day   = sprintf( $this->args['labels']['archive_day'],   get_the_time( _x( 'j', 'daily archives date format',   'redbiz' ) ) );
	...
	public function themesflat_do_week_archive_items() {
		...
		$year = sprintf( $this->args['labels']['archive_year'],  get_the_time( _x( 'Y', 'yearly archives date format', 'redbiz' ) ) );
		$week = sprintf( $this->args['labels']['archive_week'],  get_the_time( _x( 'W', 'weekly archives date format', 'redbiz' ) ) );
	...
	public function themesflat_map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format',  'redbiz' ) ) ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'redbiz' ) ) ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'redbiz' ) ) ) . '</a>';
...
class bbPress_Breadcrumb_Trail extends Breadcrumb_Trail {
	...
	public function themesflat_do_trail_items() {
		...
		elseif ( bbp_is_topic_tag_edit() ) {
			...
			if ( true === $this->args['show_title'] )
				$this->items[] = esc_html__( 'Edit', 'redbiz' );
		}
